...
class ChangePasswordUseCase @Inject constructor(private val apiService: ApiService) {

    operator fun invoke(token:String,regData: ChangePasswordRequest) = flow<State<CommonResponse>> {
        apiService.changePassword(token,regData).run {
