...
class HighFlyerRegistrationUseCase @Inject constructor(private val apiService: ApiService) {

    operator fun invoke(token:String,regData: HighFlyerRegistrationRequest) = flow<State<CommonResponse>> {
        apiService.registerHighFlyer(token,regData).run {
