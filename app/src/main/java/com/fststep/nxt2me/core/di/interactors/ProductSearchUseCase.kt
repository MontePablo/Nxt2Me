...
class ProductSearchUseCase @Inject constructor(private val apiService: ApiService) {

    operator fun invoke(token:String,data: ProductSearchRequest) = flow<State<ProductListResponse>> {

        apiService.productSearch(token,data).run {
