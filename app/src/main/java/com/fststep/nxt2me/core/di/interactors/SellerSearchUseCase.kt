...
class SellerSearchUseCase @Inject constructor(private val apiService: ApiService) {

    operator fun invoke(token:String,data: SellerSearchRequest) = flow<State<SellerListResponse>> {

        apiService.sellerSearch(token,data).run {
